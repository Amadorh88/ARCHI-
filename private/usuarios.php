...
<style>
    ...
    #modalUsuario .input-group-text { border-right: none; }
    #modalUsuario .form-control, #modalUsuario .form-select { border-left: none; }
    #modalUsuario .modal-content { border-radius: 15px; }
</style>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        const limiteGuardado = localStorage.getItem('usuarios_limite');
        if (limiteGuardado) {
    ...
    function cambiarLimite() {
        ...
        localStorage.setItem('usuarios_limite', registrosPorPagina);
        paginaActual = 1;
    ...
    function actualizarPaginacion() {
        totalPaginas = Math.ceil(usuariosFiltrados.length / registrosPorPagina);
    ...
    function renderizarPagina() {
        ...
        const paginaData = usuariosFiltrados.slice(inicio, fin);
        renderUsuarios(paginaData);
    }
    ...
    function renderUsuarios(lista) {
        ...
        if (lista.length === 0) {
            html = `<tr><td colspan="6" class="text-center text-muted py-4"><i class="bi bi-search"></i> No se encontraron resultados</td></tr>`;
        } else {
            lista.forEach(u => {
                html += `
                <tr>
    ...
    document.getElementById("formUsuario").addEventListener("submit", function (e) {
        ...
        .then(data => {
            ...
            if (esEdicion) {
                registrarActividad(`EDITÓ USUARIO: ${nombre}`, 'USUARIOS', `ID: ${id}`);
            } else {
                registrarActividad(`CREÓ USUARIO: ${nombre}`, 'USUARIOS');
            }
    ...
    function cambiarEstado(id) {
        ...
        const nombre = usuario ? usuario.nombre : 'ID:' + id;
        const nuevoEstado = usuario?.estado == 1 ? 'DESACTIVÓ' : 'ACTIVÓ';
        ...
        fetch('../api/usuarios/estado.php?id=' + id)
            .then(res => res.json())
            .then(data => {
                registrarActividad(`${nuevoEstado} USUARIO: ${nombre}`, 'USUARIOS');
                listarUsuarios();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch('../api/usuarios/eliminar.php?id=' + id)
                    .then(res => res.json())
                    .then(data => {
                        registrarActividad(`ELIMINÓ USUARIO: ${nombre}`, 'USUARIOS');
                        listarUsuarios();
